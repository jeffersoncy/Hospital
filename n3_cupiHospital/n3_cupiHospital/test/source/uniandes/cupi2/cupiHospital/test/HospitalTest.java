...
/**
 * Clase usada para verificar que los métodos de la clase Hospital están correctamente implementados.
 */
public class HospitalTest
{
	...
	/**
	 * Prueba 2: Internar un nuevo paciente.
	...
	{
		// 1. Un paciente cualquiera.
		boolean rta= hospital.internarNuevoPaciente("Unidad A", "Luis", "Suarez", 777, 27, "Golpes varios.", 7, Motivo.ACCIDENTE);
		assertEquals("La cantidad de pacientes de la unidad debería ser 1.", 1, hospital.buscarUnidad("Unidad A").darPacientes().size());
		...
		// 2. Un paciente de geriatría.
		rta= hospital.internarNuevoPaciente("Unidad E", "Juan", "Lopez", 333, 62, "Demencia senil.", 1000, Motivo.TRATAMIENTO);
		assertEquals("La cantidad de pacientes de la unidad debería ser 1.", 1, hospital.buscarUnidad("Unidad E").darPacientes().size());
		...
		assertTrue ("La respuesta debería ser verdadero", rta);
		rta= hospital.internarNuevoPaciente("Unidad E", "Juan2", "Lopez2", 334, 63, "Demencia senil.", 1000, Motivo.TRATAMIENTO);
		assertEquals("La cantidad de pacientes de la unidad debería ser 2.", 2, hospital.buscarUnidad("Unidad E").darPacientes().size());
		...
		// 3. Un paciente de pediatría.
		rta= hospital.internarNuevoPaciente("Unidad C", "Paola", "Lopez", 336, 12, "Gripa fuerte.", 5, Motivo.INFECCION);
		assertEquals("La cantidad de pacientes de la unidad debería ser 1.", 1, hospital.buscarUnidad("Unidad C").darPacientes().size());
		...
		assertTrue ("La respuesta debería ser verdadero", rta);
		rta= hospital.internarNuevoPaciente("Unidad C", "Paola2", "Lopez2", 337, 14, "Gripa fuerte.", 5, Motivo.INFECCION);
		assertEquals("La cantidad de pacientes de la unidad debería ser 2.", 2, hospital.buscarUnidad("Unidad C").darPacientes().size());
		...
		rta = hospital.internarNuevoPaciente("Unidad A", "Luis", "Suarez", 777, 27, "Golpes varios.", 7, Motivo.ACCIDENTE);
		assertEquals("La cantidad de pacientes de la unidad debería ser 1.", 1, hospital.buscarUnidad("Unidad A").darPacientes().size());
		assertEquals("La cantidad total de pacientes debería ser 5", 5, hospital.darCantidadTotalPacientes());
		assertFalse ("La respuesta debería ser falso", rta);

		// 4b. Es mayor de la edad necesaria para pediatría.
		rta= hospital.internarNuevoPaciente("Unidad C", "Paola", "Lopez", 346, 22, "Gripa fuerte.", 5, Motivo.INFECCION);
		assertEquals("La cantidad de pacientes de la unidad debería ser 2.", 2, hospital.buscarUnidad("Unidad C").darPacientes().size());
		assertEquals("La cantidad total de pacientes debería ser 5", 5, hospital.darCantidadTotalPacientes());
		assertFalse ("La respuesta debería ser falso", rta);

		// 4c. Es menor de la edad necesaria para geriatría.
		rta= hospital.internarNuevoPaciente("Unidad E", "Juan", "Lopez", 4446, 22, "Gripa fuerte.", 5, Motivo.INFECCION);
		assertEquals("La cantidad de pacientes de la unidad debería ser 2.", 2, hospital.buscarUnidad("Unidad C").darPacientes().size());
		assertEquals("La cantidad total de pacientes debería ser 5", 5, hospital.darCantidadTotalPacientes());
		assertFalse ("La respuesta debería ser falso", rta);
	...
	/**
	 * Prueba 3: Buscar un paciente por ID.
	...
	/**
	 * Prueba 4: Dar de alta a un paciente.
	 * 1. Cuando el paciente no existe.
	 * 2. Cuando el paciente si existe.
	 */
	@Test
	...
	/**
	 * Prueba 5: Reubicar a un paciente.
	 * 1. Cuando el paciente no existe.
	 * 2. Cuando el paciente si existe.
	 * 3. Cuando la unidad de destino es la misma que la de origen. 
	 * 4. Cuando el paciente: existe, se intenta reubicar en pediatría y no tiene la edad apropiada.
	 * 5. Cuando el paciente: existe, se intenta reubicar en geriatría y no tiene la edad apropiada.
	 */
	@Test
	public void testReubicarPaciente()
	{
		...
		assertNull("El paciente no debería estar en la Unidad A", hospital.buscarUnidad("Unidad A").buscarPaciente(777));
		assertNotNull("El paciente debería estar en la Unidad B", hospital.buscarUnidad("Unidad B").buscarPaciente(777));
		...
		assertFalse("No se debería poder reubicar el paciente. La edad sobrepasa la edad máxima para atención en la unidad de pediatría", rta3);
		assertNull("El paciente no debería estar en la Unidad C", hospital.buscarUnidad("Unidad C").buscarPaciente(333));
		assertNotNull("El paciente debería estar en la Unidad E", hospital.buscarUnidad("Unidad E").buscarPaciente(333));
		...
		assertFalse("No se debería poder reubicar el paciente. No cumple con la edad mínima requerida para atención en la unidad de geriatría", rta4);
		assertNull("El paciente no debería estar en la Unidad C", hospital.buscarUnidad("Unidad C").buscarPaciente(333));
		assertNotNull("El paciente debería estar en la Unidad E", hospital.buscarUnidad("Unidad E").buscarPaciente(333));
	...
	/**
	 * Prueba 6: Buscar la unidad a la que pertenece un paciente.
	 * 1. Cuando el paciente no existe.
	 * 2. Cuando el paciente si existe.
	 */
	@Test
	public void testBuscarUnidadPaciente()
	{
		...
		assertEquals("La unidad debería ser: Unidad A.", "Unidad A", rta2.darNombre());
	}

	/**
	 * Prueba 7: Buscar una unidad dado su nombre.
	 * 1. La unidad existe.
	 * 2. La unidad no existe.
	 */
	@Test
	public void testBuscarUnidad()
	{
		assertNotNull("Debería existir la unidad", hospital.buscarUnidad("Unidad D"));
		assertEquals("No corresponde el tipo de la unidad buscada", Tipo.MATERNIDAD, hospital.buscarUnidad("Unidad D").darTipo());
	...
	public void testDarCantidadTotalPacientes()
	{
		hospital.internarNuevoPaciente("Unidad A", "Luis", "Suarez", 777, 27, "Golpes varios.", 7, Motivo.ACCIDENTE);
		assertEquals("La cantidad total de pacientes debería ser 1", 1, hospital.darCantidadTotalPacientes());
		hospital.internarNuevoPaciente("Unidad E", "Juan", "Lopez", 333, 62, "Demencia senil.", 1000, Motivo.TRATAMIENTO);
		assertEquals("La cantidad total de pacientes debería ser 2", 2, hospital.darCantidadTotalPacientes());
		hospital.internarNuevoPaciente("Unidad E", "Juan2", "Lopez2", 334, 63, "Demencia senil.", 1000, Motivo.TRATAMIENTO);
		assertEquals("La cantidad total de pacientes debería ser 3", 3, hospital.darCantidadTotalPacientes());
	...
	public void testDarPacienteMayorEdad()
	{
		...
		assertEquals("La edad del paciente de mayor edad no corresponde", 63, hospital.darPacienteMayorEdad().darEdad());
		
	}

	/**
	 * Prueba 10. Asignar el encargado de una unidad.
	 * 1. la unidad existe y no tiene encargado.
	 * 2. la unidad existe y tiene encargado.
	 * 3. la unidad no existe
	 */
	@Test
